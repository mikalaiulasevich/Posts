...
## Стартовый промпт
Первичный artifact приложен в репозитории как `prompt.md`. Ключевая выдержка стартового промпта:
...
Полный текст сохранён в `prompt.md`, чтобы проверяющий видел исходные правила без пересказа.
